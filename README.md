# f-R-Cosmology---Hu-Sawicki-Model
import numpy as np
...
for b in b_space:
    for lamb in lamb_space:
        ...
        ax.plot(z[sort_idx], H0 * E_sol.x[sort_idx], label = rf"b = {b}, $\lambda$ = {lamb}")
z_data = np.array([
...
ax.set_title(r"$b$ vs $\Lambda$ from Chebyshev Method")
ax.set_ylabel('$H(z)$')
...
def log_likelihood(b,lamb,om,H_vals,z_data,H_data,err_data):
    E_g = np.sqrt(om*(1+z)**3 + (1-om))
    res_func = r_residual(b, lamb)
    E_sol = root(res_func, E_g, method='lm')
    E_interp = interp1d(z, E_sol.x, kind='cubic')
    H_model_at_data = H_vals * E_interp(z_data)
    chi2 = np.sum(((H_data - H_model_at_data) / err_data) ** 2)
    return -0.5 * chi2
def log_prior2(b,lamb,om,H_vals):
    h0prior = -(H_vals-70)**2/2**2
    omprior = -(om-0.3)**2/0.1**2
    bprior = -(b-0.01)**2/0.02**2
    lambprior = -(lamb-1.2)**2/0.1**2
    return h0prior+omprior+bprior+lambprior
def log_posterior(b,lamb,om,H_vals,z_data,H_data,err_data):
    return log_likelihood(b,lamb,om,H_vals,z_data,H_data,err_data) + log_prior2(b,lamb,om,H_vals)

b = np.linspace(0.01,1,50)
pp=[]
for b_vals in b:
    l = log_posterior(b_vals,1.2,0.3,70,z_data, H_data, err_data)
    pp.append(l)
plt.plot(b,pp)
plt.show()
lamb = np.linspace(0.5,2,50)
ppl = []
for lamb_vals in lamb:
    l = log_posterior(0.01,lamb_vals,0.3,70,z_data, H_data, err_data)
    ppl.append(l)
plt.plot(lamb,ppl)
plt.show()
H_vals = np.linspace(55,75,60)
pph = []
for H in H_vals:
    l = log_posterior(0.01,1.2,0.3,H,z_data, H_data, err_data)
    pph.append(l)
plt.plot(H_vals,pph)
plt.show()
Om_vals = np.linspace(0,1,100)
ppo = []
for Om in Om_vals:
    l = log_posterior(0.01,1.2,Om,70,z_data, H_data, err_data)
    ppo.append(l)
plt.plot(Om_vals,ppo)
plt.show()
